...
const OrderDetailsDialog = ({
  ...
  const displayTableNumber =
    tableId.length > 10
      ? `Table ${tableId.slice(-3)}` // Show last 3 characters if it's a long ID
      : tableId;
  ...
  useEffect(() => {
    if (open) {
      ...
      fetch(`http://localhost:3001/orders/${restaurantId}/table/${tableId}`)
        .then((res) => {
          if (!res.ok) {
            // Handle 404 and other HTTP errors
            throw new Error(`HTTP error! status: ${res.status}`);
        ...
        .then((data) => {
          ...
          if (
            ...
            const transformedOrder = {
              ...
              table_number: orderData.table_number || tableId,
            };
  ...
  }, [open, tableId, displayTableNumber, restaurantId]);
