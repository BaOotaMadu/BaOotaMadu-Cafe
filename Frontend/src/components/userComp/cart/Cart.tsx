...
const Cart: React.FC<CartProps> = ({ isOpen, onClose }) => {
  ...
  const [billToken, setBillToken] = useState<number | null>(null);
          ...
          try {
            ...
            if (verifyRes.data.success) {
              ...
              if (orderRes.status === 201) {
                const token = orderRes.data.order.tokenNumber;

                // ✅ CRITICAL: Save ONLY after order is confirmed
                localStorage.setItem("latestOrderToken", String(token));
  ...
  return (
    <>
      {/* ✅ Bill dialog at top level */}
      {showBillDialog && billToken !== null && (
        <BillComp
          open={showBillDialog}
